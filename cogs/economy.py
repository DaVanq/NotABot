...
PAYOUTS = {
    (SMReel.two, SMReel.two, SMReel.six) : {
        ...
        "phrase" : "OH MAH GOD! JACKPOT! 226! Your bid has been multiplied * 2500! :tada:"
    },
    (SMReel.flc, SMReel.flc, SMReel.flc) : {
        "payout" : lambda x: x + 1000,
        "phrase" : "4LC! +1000! :tada:"
    },
    (SMReel.cherries, SMReel.cherries, SMReel.cherries) : {
        "payout" : lambda x: x + 800,
        "phrase" : "Three cherries! +800! :tada:"
    },
    (SMReel.two, SMReel.six) : {
        "payout" : lambda x: x * 4 + x,
        "phrase" : "2 6! Your bid has been multiplied * 4! :tada:"
    },
    (SMReel.cherries, SMReel.cherries) : {
        "payout" : lambda x: x * 3 + x,
        "phrase" : "Two cherries! Your bid has been multiplied * 3! :tada:"
    },
    "3 symbols" : {
        "payout" : lambda x: x + 500,
        "phrase" : "Three symbols! +500! :tada:"
    },
    "2 symbols" : {
        "payout" : lambda x: x * 2 + x,
        "phrase" : "Two consecutive symbols! Your bid has been multiplied * 2! :tada:"
    },
}
...
class Economy:
    ...
    async def transfer(self, ctx, user: discord.Member, sum: int):
        ...
        try:
            ...
            await self.bot.say("{} credits have been transferred to {}'s"
                               " account! What a generous soul!".format(sum, user.name))
        except NegativeValue:
            await self.bot.say("You need to transfer at least 1 credit silly goose :joy:")
        except SameSenderAndReceiver:
            await self.bot.say("You can't transfer credits to yourself :face_palm:")
        except InsufficientBalance:
            await self.bot.say("You don't have that sum in your bank account. :sob:")
        except NoAccount:
            await self.bot.say("Uhh...That user has no bank account.")
    ...
    async def reset(self, ctx, confirmation: bool=False):
        ...
        if confirmation is False:
            await self.bot.say("**CAREFUL!**This will delete all bank accounts on "
                               "this server.\nIf you're sure, type "
    ...
    async def payday(self, ctx):  # TODO
        ...
        if self.bank.account_exists(author):
            if id in self.payday_register[server.id]:
                ...
                if seconds >= self.settings[server.id]["PAYDAY_TIME"]:
                    ...
                    self.payday_register[server.id][
                        id] = int(time.perf_counter())
                    await self.bot.say(
                        "{} Ohhh yeaa! Take some imaginary $$$ Enjoy! (+{}"
                        " credits! :tada:)".format(
                            author.mention,
                    ...
                    await self.bot.say(
                        "{} Woah. Too soon pal. For your next payday you have to"
            ...
            await self.bot.say("{} Boop, beep! You need an account to receive credits."
    ...
    async def _server_leaderboard(self, ctx, top: int=10):
        ...
        if highscore != "":
            ...
            await self.bot.say(":face_palm: There are no accounts in the bank.")
    ...
    async def slot(self, ctx, bid: int):
        ...
        try:
            ...
            await self.bot.say("{} Hey! You need an account to use my fancy slot "
                               "machine and gamble! Type `{}bank register` to open one."
                               "".format(author.mention, ctx.prefix))
        except InsufficientBalance:
            await self.bot.say("{} Silly you! You need an account with **enough** funds to "
                               "play the slot machine ~~and give me more $$~~.".format(author.mention))
        except OnCooldown:
            await self.bot.say("Woahh! Slot machine is still cooling off! Wait {} "
                               "seconds between each pull".format(slot_time))
        except InvalidBid:
            await self.bot.say("Wait a minute buddy! Bid must be between {} and {}."
