...
class Audio:
    ...
    async def _add_song_status(self, song):
        ...
        log.debug('NotABot status has changed to song title: ' + song.title)
    ...
    async def _play(self, sid, url, channel):
        ...
        if self._valid_playable_url(url) or "[SEARCH:]" in url:
            ...
            try:
                ...
                message = ("Uh oh! I'm unable to play '{}' because of an error:\n"
                          "'{}'".format(clean_url, str(e)))
                message = escape(message, mass_mentions=True)
                await self.bot.send_message(channel, message)
                return
            except MaximumLength:
                message = ("Woops! I'm unable to play '{}' because it exceeds the "
                          "maximum audio length! Please play something a bit shorter O.O".format(clean_url))
                message = escape(message, mass_mentions=True)
                await self.bot.send_message(channel, message)
                return
            local = False
    ...
    async def audioset_volume(self, ctx, percent: int=None):
        ...
        if percent is None:
            ...
            msg = "Erm...Volume must be between 0 and 100."
    ...
    async def play_local(self, ctx, *, name):
        ...
        if not self.voice_connected(server):
            try:
                ...
                await self.bot.say("Hahahha.. you must join a voice channel before I can"
                                   " play anything!")
                return
            except UnauthorizedConnect:
                await self.bot.say("I don't have permissions to join your"
                                   " voice channel. Might wanna adjust that..")
                return
            except UnauthorizedSpeak:
                await self.bot.say("Erm..I don't have permissions to speak in your"
                                   " voice channel. Change pls.")
                return
            except ChannelUserLimit:
                await self.bot.say("ERROR! Your voice channel is full.")
                return
            else:
                await self._join_voice_channel(voice_channel)
        ...
        if self.is_playing(server):
            await self.bot.say("Hey! I'm already playing a song on this server!")
        ...
        if self.currently_downloading(server):
            await self.bot.say("Unfortunately, you need to wait for the previous song to load since I'm already downloading a file!")
            return
    ...
    async def repeat_toggle(self, ctx):
        ...
        if repeat:
            ...
            await self.bot.say("Awww...repeat toggled off.")
            ...
            if msg.author.voice_channel == vchan:
                if self.can_instaskip(msg.author):
                    ...
                    if vote >= thresh:
                        ...
                        await self.bot.say("Vote threshold has been met. Skipping...")
                        return
                    else:
