...
class CustomCommands:
    ...
    async def cc_add(self, ctx, command : str, *, text):
        ...
        if command not in cmdlist:
            ...
            await self.bot.say("Woohoo! Custom command successfully added! :tada:")
        else:
    ...
    async def cc_edit(self, ctx, command : str, *, text):
        ...
        if server.id in self.c_commands:
            ...
            if command in cmdlist:
                ...
                self.c_commands[server.id] = cmdlist
                dataIO.save_json(self.file_path, self.c_commands)
                await self.bot.say("Yay! Custom command successfully edited! :tada:")
            else:
                await self.bot.say("That command apparently doesn't exist. Use "
    ...
    async def cc_delete(self, ctx, command : str):
        ...
        if server.id in self.c_commands:
            ...
            if command in cmdlist:
                ...
                self.c_commands[server.id] = cmdlist
                dataIO.save_json(self.file_path, self.c_commands)
                await self.bot.say("Custom command successfully deleted :sob:")
            else:
