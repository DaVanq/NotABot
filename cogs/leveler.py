...
class Leveler:
    ...
    async def profile(self,ctx, *, user : discord.Member=None):
        ...
        if server.id in self.settings["disabled_servers"]:
            await self.bot.say("**Leveler commands for this server are disabled D:**")
            return
    ...
    async def rep(self, ctx, user : discord.Member = None):
        ...
        if user and user.id == org_user.id:
            await self.bot.say("**You can't give a rep to yourself! :face_palm:**")
            return
        if user and user.bot:
            await self.bot.say("**Silly!! You can't give a rep to a bot! :face_palm:**")
            return
        ...
        if user and delta >= 43200.0 and delta>0:
            ...
            await self.bot.say("**Congrats! You have just given {} a reputation point! Have a cookie :cookie:**".format(self._is_mention(user)))
        else:
            # calulate time left
            seconds = 43200 - delta
            if seconds < 0:
                await self.bot.say("**Yay! You can give a rep! :tada:**")
                return
