...
class Trivia:
    ...
    async def botplays(self, ctx):
        ...
        if self.settings[server.id]["BOT_PLAYS"]:
            ...
            await self.bot.say("Alright, I won't embarass you at trivia anymore :joy:")
        else:
            self.settings[server.id]["BOT_PLAYS"] = True
            await self.bot.say("I'll gain a point every time you don't answer in time >:D")
        self.save_settings()
    ...
    async def trivia(self, ctx, list_name: str):
        ...
        if not session:
            try:
                ...
                await self.bot.say("Uh oh! That trivia list doesn't exist, but if you want it to exist, put it into #suggestions ")
            except Exception as e:
                print(e)
                await self.bot.say("Woops. Error loading that trivia list.")
            else:
                settings = self.settings[server.id]
            ...
            await self.bot.say("Wait a second.. a trivia session is already ongoing in this channel. Why don't ya join in!")
    ...
    async def trivia_stop(self, ctx):
        ...
        if session:
            if author == session.starter or is_authorized:
                ...
                await self.bot.say("Awww...the trivia session has stopped :cry:")
            else:
                await self.bot.say("You are not allowed to do that!")
        else:
            await self.bot.say("Hey!T here's no trivia session ongoing in this channel. Start one!")
    ...
    async def trivia_list(self):
        ...
        if lists:
            ...
            await self.bot.say("Error! There are no trivia lists available.")
...
class TriviaSession():
    ...
    async def new_question(self):
        ...
        while self.status != "correct answer" and abs(self.timer - int(time.perf_counter())) <= self.settings["DELAY"]:
            if abs(self.timeout - int(time.perf_counter())) >= self.settings["TIMEOUT"]:
                await self.bot.say("Guys...? Where'd you run off to? Well, I guess I'll stop then :cry:")
    ...
    async def check_answer(self, message):
        ...
        if has_guessed:
            ...
            msg = "You got it {}! **+1** to you! :tada:".format(message.author.name)
